/**
=========================================================
* GestiaSoft - Grupos Page
=========================================================
* Gestión de grupos del sistema
*/
...
// Grupo Detail Component
import GrupoDetail from "./GrupoDetail";

// Images 
import grupo_logo from "assets/images/users_logo.png";
...
function Grupos() {
    ...
    // Estado para el panel lateral de grupo
    const [grupoPanel, setGrupoPanel] = useState({
        open: false,
    ...
    // Datos de ejemplo para la tabla según estructura de BD seg_Grupos
    const [data, setData] = useState([
        {
            grupo_id: 1,
            codigo: "ADMIN",
            nombre: "Administradores",
            descripcion: "Grupo de usuarios administradores del sistema",
            status: true,
        ...
        },
        {
            ...
            codigo: "USERS",
            nombre: "Usuarios",
            descripcion: "Grupo de usuarios estándar del sistema",
            status: true,
        ...
        },
        {
            ...
            codigo: "GUEST",
            nombre: "Invitados",
            descripcion: "Grupo de usuarios invitados con acceso limitado",
            status: true,
        ...
        },
        {
            ...
            codigo: "MODERATOR",
            nombre: "Moderadores",
            descripcion: "Grupo de usuarios moderadores del sistema",
            status: false,
        ...
        },
        {
            ...
            codigo: "SUPPORT",
            nombre: "Soporte",
            descripcion: "Grupo de usuarios de soporte técnico",
            status: true,
    ...
    // Función para filtrar grupos por búsqueda y filtros
    const filteredGrupos = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(grupo =>
                ...
                grupo.codigo?.toLowerCase().includes(searchLower) ||
                grupo.descripcion?.toLowerCase().includes(searchLower)
            );
        }

        // Filtro por estado
        if (statusFilter !== "all") {
            filtered = filtered.filter(grupo => grupo.status === (statusFilter === "true"));
        }

        return filtered;
    }, [data, searchTerm, statusFilter]);

    // Función para obtener solo los grupos de la página actual
    const paginatedGrupos = useMemo(() => {
        const startIndex = pagination.pageIndex * pagination.pageSize;
        const endIndex = startIndex + pagination.pageSize;
        return filteredGrupos.slice(startIndex, endIndex);
    }, [filteredGrupos, pagination.pageIndex, pagination.pageSize]);
    ...
    const handleDeleteConfirm = () => {
        if (deleteAlert.grupo) {
            setData(data.filter(grupo => grupo.grupo_id !== deleteAlert.grupo.grupo_id));
            setDeleteAlert({ open: false, grupo: null });
        }
    };
    ...
    // Funciones para manejar el panel lateral de grupo
    ...
    const handleGrupoSave = async (grupoData) => {
        setGrupoPanel(prev => ({ ...prev, loading: true }));

        try {
            // Simular guardado (aquí iría la llamada a la API)
            await new Promise(resolve => setTimeout(resolve, 1000));

            if (grupoPanel.mode === "create") {
                // Crear nuevo grupo
                const newGrupo = {
                    ...grupoData,
                    grupo_id: Math.max(...data.map(g => g.grupo_id)) + 1,
                    created_at: new Date().toISOString(),
                    created_by: 1,
                    updated_at: new Date().toISOString(),
                    updated_by: 1
                };
                setData([...data, newGrupo]);
            } else {
                // Actualizar grupo existente
                setData(data.map(grupo =>
                    grupo.grupo_id === grupoData.grupo_id
                        ? { ...grupo, ...grupoData, updated_at: new Date().toISOString(), updated_by: 1 }
                        : grupo
                ));
            }

            handleGrupoPanelClose();
        } catch (error) {
            console.error("Error al guardar grupo:", error);
        } finally {
            setGrupoPanel(prev => ({ ...prev, loading: false }));
        }
    };
    ...
    const CodigoCell = ({ cell }) => (
        ...
        >
            🏷️ {cell.getValue() || "Sin código"}
        </span>
    );

    CodigoCell.propTypes = {
        cell: PropTypes.shape({
            getValue: PropTypes.func.isRequired
        }).isRequired
    };
    ...
    const ActionsCell = ({ row }) => (
        <div style={{
            ...
            <IconButton
                size="medium"
                color="primary"
                onClick={() => handleEditGrupo(row.original)}
                sx={{
    ...
    const tableOptions = {
        ...
        rowCount: filteredGrupos.length,
    ...
    return (
        <AppPageLayout>
                                ...
                                <CardContent sx={{ padding: 0 }}>
                                    <SoftBox p={{ xs: 2, sm: 3, md: 4 }}>
                                            ...
                                            <SoftBox display="flex" alignItems="center" gap={2}>
                                                ...
                                                <SoftBox>
                                                    ...
                                                    <SoftTypography variant="button" color="text" fontWeight="medium" opacity={0.8}>
                                                        Administración de grupos del sistema {APP_NAME}
                                                    </SoftTypography>
                                                </SoftBox>
                                            </SoftBox>

                                            {/* Lado derecho: Nuevo Grupo */}
                                            <Button
                                                variant="contained"
                                                color="primary"
                                                size="large"
                                        ...
                                        >
                                            ...
                                            >
                                                📊 Mostrando <strong style={{ color: '#1976d2' }}>{filteredGrupos.length}</strong> de <strong style={{ color: '#1976d2' }}>{data.length}</strong> grupos
                                                {(searchTerm || statusFilter !== "all") && (
                                            ...
                                            <Grid container spacing={{ xs: 1, sm: 2, md: 3 }} alignItems="center">
                                                ...
                                                <Grid item xs={12} md={8}>
                                                    ...
                                                    >
                                                        <input
                                                            type="text"
                                                            placeholder="Buscar por nombre, código o descripción del grupo..."
                                                            value={searchTerm}
            ...
            {/* Panel lateral para crear/editar grupos */}
            <SidePanelRight
                open={grupoPanel.open}
                onClose={handleGrupoPanelClose}
                title={grupoPanel.mode === "create" ? "Nuevo Grupo" : "Editar Grupo"}
                subtitle={grupoPanel.mode === "create"
                    ? "Crear un nuevo grupo en el sistema"
                    : `Editando grupo: ${grupoPanel.grupo?.nombre || ""}`
                }
            >
                <GrupoDetail
                    grupo={grupoPanel.grupo}
                    mode={grupoPanel.mode}
                    onSave={handleGrupoSave}
                    onCancel={handleGrupoPanelClose}
                    loading={grupoPanel.loading}
                />
            </SidePanelRight>
            ...
            <ConfirmAlert
                ...
                cancelText="Cancelar"
                severity="error"
                itemName={deleteAlert.grupo?.nombre}
                showItemName={true}
                itemLabel="Grupo"
            />
        </AppPageLayout>
