/**
=========================================================
* GestiaSoft - RolPermiso Detail Component
=========================================================
* Formulario para crear/editar asignaciones de permisos a roles
*/
...
function RolPermisoDetail({ rolPermiso, mode, onSave, onCancel, loading }) {
    ...
    // Datos de ejemplo para roles y permisos (en producción vendrían de la API)
    const roles = [
    ...
    // Inicializar formulario cuando cambie el rolPermiso o mode
    useEffect(() => {
        // ✅ LIMPIAR ERRORES CADA VEZ QUE SE ENTRE A LA PANTALLA
        setErrors({});
        setTouched({});

        if (mode === "edit" && rolPermiso) {
            setFormData({
                rol_id: rolPermiso.rol_id || "",
                rol_nombre: rolPermiso.rol_nombre || "",
                permiso_id: rolPermiso.permiso_id || "",
                permiso_nombre: rolPermiso.permiso_nombre || "",
                status: rolPermiso.status !== undefined ? rolPermiso.status : true
            });
        } else {
    ...
    const validateForm = () => {
        ...
        Object.keys(formData).forEach(key => {
            if (key !== "rol_nombre" && key !== "permiso_nombre") { // Solo validar campos editables
    ...
    const handleSubmit = (e) => {
        ...
        const allTouched = {};
        Object.keys(formData).forEach(key => {
            if (key !== "rol_nombre" && key !== "permiso_nombre") {
                allTouched[key] = true;
            }
        });
        ...
        if (validateForm()) {
            const submitData = {
                ...formData,
                rol_permiso_id: mode === "edit" ? rolPermiso.rol_permiso_id : undefined
            };
            onSave(submitData);
        }
    };
    ...
    const handleRolChange = (rolId) => {
        ...
        setTouched(prev => ({ ...prev, rol_id: true }));
    };
    ...
    const hasAnyInput = formData.rol_id || formData.permiso_id;
                                        ...
                                        <>
                                            ...
                                            <Grid item xs={12} md={6}>
                                                <SoftBox display="flex" flexDirection="column" gap={1}>
                                                    ...
                                                    <TextField
                                                        fullWidth
                                                        value={rolPermiso.created_at ? new Date(rolPermiso.created_at).toLocaleString('es-ES') : "N/A"}
                                                        variant="outlined"
                                                        size="small"
                                                        InputProps={{
                                                            readOnly: true,
                                            ...
                                            <Grid item xs={12} md={6}>
                                                <SoftBox display="flex" flexDirection="column" gap={1}>
                                                    ...
                                                    <TextField
                                                        fullWidth
                                                        value={rolPermiso.updated_at ? new Date(rolPermiso.updated_at).toLocaleString('es-ES') : "N/A"}
                                                        variant="outlined"
                                                        size="small"
                                                        InputProps={{
                                                            readOnly: true,
            ...
            <SoftBox mt={4}>
                <Alert severity="info" sx={{ borderRadius: '8px' }}>
                    <SoftTypography variant="body2" fontWeight="medium">
                        ...
                        tengan acceso al permiso especificado. Asegúrate de que la combinación rol-permiso sea única en el sistema.
                    </SoftTypography>
                </Alert>
            </SoftBox>
        </SoftBox>
...
RolPermisoDetail.propTypes = {
    rolPermiso: PropTypes.shape({
        rol_permiso_id: PropTypes.number,
...
// Valores por defecto
RolPermisoDetail.defaultProps = {
    rolPermiso: null,
    loading: false
};

export default RolPermisoDetail;
